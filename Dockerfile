...
RUN CGO_ENABLED=0 GOOS=linux go build -o /bin/releases-notification ./cmd/notification
...
FROM alpine:3.21 AS notification-runtime
RUN adduser -D -g '' appuser
USER appuser
WORKDIR /app
COPY --from=build /bin/releases-notification /app/releases-notification
ENTRYPOINT ["/app/releases-notification"]
