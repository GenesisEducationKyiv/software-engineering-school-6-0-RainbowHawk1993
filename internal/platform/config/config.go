package config

import (
	"fmt"
	"os"
	"strconv"
	"strings"
	"time"

	"releasesapi/internal/modules/notification"
)

type RedisConfig struct {
	Addr     string
	Password string
	DB       int
}

type APIConfig struct {
	Port              string
	GRPCPort          string
	InternalGRPCPort  string
	APIKey            string
	DatabaseURL       string
	GitHubToken       string
	AppBaseURL        string
	SMTP              notification.SMTPConfig
	Redis             RedisConfig
}

type ScannerConfig struct {
	SubscriptionAPIGRPCAddr string
	APIKey                  string
	GitHubToken             string
	AppBaseURL              string
	ScanInterval            time.Duration
	SMTP                    notification.SMTPConfig
	Redis                   RedisConfig
}

func LoadAPI() (APIConfig, error) {
	smtpPort, err := parseSMTPPort()
	if err != nil {
		return APIConfig{}, err
	}

	redisDB, err := parseRedisDB()
	if err != nil {
		return APIConfig{}, err
	}

	return APIConfig{
		Port:             valueOrDefault("PORT", "8080"),
		GRPCPort:         valueOrDefault("GRPC_PORT", "9090"),
		InternalGRPCPort: valueOrDefault("INTERNAL_GRPC_PORT", "9091"),
		APIKey:           valueOrDefault("API_KEY", "dev-api-key"),
		DatabaseURL:      valueOrDefault("DATABASE_URL", "postgres://postgres:postgres@db:5432/releases?sslmode=disable"),
		GitHubToken:      strings.TrimSpace(os.Getenv("GITHUB_TOKEN")),
		AppBaseURL:       strings.TrimRight(valueOrDefault("APP_BASE_URL", "http://localhost:8080"), "/"),
		SMTP: notification.SMTPConfig{
			Host:     valueOrDefault("SMTP_HOST", "mailpit"),
			Port:     smtpPort,
			Username: strings.TrimSpace(os.Getenv("SMTP_USERNAME")),
			Password: strings.TrimSpace(os.Getenv("SMTP_PASSWORD")),
			From:     valueOrDefault("SMTP_FROM", "noreply@releases-api.local"),
		},
		Redis: RedisConfig{
			Addr:     valueOrDefault("REDIS_ADDR", "redis:6379"),
			Password: strings.TrimSpace(os.Getenv("REDIS_PASSWORD")),
			DB:       redisDB,
		},
	}, nil
}

func LoadScanner() (ScannerConfig, error) {
	scanInterval, err := parseScanInterval()
	if err != nil {
		return ScannerConfig{}, err
	}

	smtpPort, err := parseSMTPPort()
	if err != nil {
		return ScannerConfig{}, err
	}

	redisDB, err := parseRedisDB()
	if err != nil {
		return ScannerConfig{}, err
	}

	return ScannerConfig{
		SubscriptionAPIGRPCAddr: valueOrDefault("SUBSCRIPTION_API_GRPC_ADDR", "api:9091"),
		APIKey:                  valueOrDefault("API_KEY", "dev-api-key"),
		GitHubToken:             strings.TrimSpace(os.Getenv("GITHUB_TOKEN")),
		AppBaseURL:              strings.TrimRight(valueOrDefault("APP_BASE_URL", "http://localhost:8080"), "/"),
		ScanInterval:            scanInterval,
		SMTP: notification.SMTPConfig{
			Host:     valueOrDefault("SMTP_HOST", "mailpit"),
			Port:     smtpPort,
			Username: strings.TrimSpace(os.Getenv("SMTP_USERNAME")),
			Password: strings.TrimSpace(os.Getenv("SMTP_PASSWORD")),
			From:     valueOrDefault("SMTP_FROM", "noreply@releases-api.local"),
		},
		Redis: RedisConfig{
			Addr:     valueOrDefault("REDIS_ADDR", "redis:6379"),
			Password: strings.TrimSpace(os.Getenv("REDIS_PASSWORD")),
			DB:       redisDB,
		},
	}, nil
}

func parseScanInterval() (time.Duration, error) {
	scanInterval := 15 * time.Minute
	if raw := strings.TrimSpace(os.Getenv("SCAN_INTERVAL")); raw != "" {
		parsed, err := time.ParseDuration(raw)
		if err != nil {
			return 0, fmt.Errorf("parse SCAN_INTERVAL: %w", err)
		}
		scanInterval = parsed
	}
	return scanInterval, nil
}

func parseSMTPPort() (int, error) {
	smtpPort := 1025
	if raw := strings.TrimSpace(os.Getenv("SMTP_PORT")); raw != "" {
		parsed, err := strconv.Atoi(raw)
		if err != nil {
			return 0, fmt.Errorf("parse SMTP_PORT: %w", err)
		}
		smtpPort = parsed
	}
	return smtpPort, nil
}

func parseRedisDB() (int, error) {
	redisDB := 0
	if raw := strings.TrimSpace(os.Getenv("REDIS_DB")); raw != "" {
		parsed, err := strconv.Atoi(raw)
		if err != nil {
			return 0, fmt.Errorf("parse REDIS_DB: %w", err)
		}
		redisDB = parsed
	}
	return redisDB, nil
}

func valueOrDefault(key, fallback string) string {
	value := strings.TrimSpace(os.Getenv(key))
	if value == "" {
		return fallback
	}

	return value
}
