...
func (s *PostgresSubscriptionStore) CreateSubscription(ctx context.Context, params ports.CreateSubscriptionParams) (domain.Subscription, error) {
	...
	if errors.As(err, &pgErr) && pgErr.Code == "23505" {
		return domain.Subscription{}, apperr.ErrAlreadySubscribed
	}
...
func (s *PostgresSubscriptionStore) ConfirmByToken(ctx context.Context, token string) (domain.Subscription, error) {
	subscription, err := scanSubscription(s.db.QueryRow(ctx, `
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return domain.Subscription{}, apperr.ErrTokenNotFound
	}
...
func (s *PostgresSubscriptionStore) ListConfirmedByEmail(ctx context.Context, email string) ([]domain.Subscription, error) {
...
func (s *PostgresSubscriptionStore) ListConfirmedForScan(ctx context.Context) ([]domain.Subscription, error) {
	rows, err := s.db.Query(ctx, `
		select id, email, repo_owner, repo_name, confirmed, confirm_token, unsubscribe_token, last_seen_tag, created_at, updated_at
		from subscriptions
